...
for i=1, #fixed_wide_templates do
	fixed_wide_templates[i] = require(fixed_wide_templates[i])

	mod:hook(fixed_wide_templates[i], "create_widget_defintion", function (func, template, scenegraph_id)
		local widget = func(template, scenegraph_id)
		local style = widget.style
	...
	mod:hook_origin(fixed_wide_templates[i], "update_function", function (parent, ui_renderer, widget, template, crosshair_settings, dt, t, draw_hit_indicator)
		local style = widget.style
		local hit_progress, hit_color, hit_weakspot = parent:hit_indicator()
